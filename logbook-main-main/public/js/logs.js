...
class LogsManager {
    ...
    setupEventListeners() {
        ...
        if (bulkApproveBtn) {
            bulkApproveBtn.addEventListener('click', () => this.bulkUpdateStatus('Pending'));
        }
    ...
    filterEntries() {
        ...
        this.filteredEntries = this.entries.filter(entry => {
            ...
            // Document status filter: In / Pending / Out
            let matchesDocStatus = true;
            if (docStatusValue) {
                const rawDs = entry.docStatus || 'In';
                const ds = rawDs.toLowerCase() === 'incoming' ? 'In' : (rawDs.toLowerCase() === 'pending' || rawDs.toLowerCase() === 'in-service' ? 'Pending' : rawDs);
                if (docStatusValue === 'In')      matchesDocStatus = ds === 'In';
                else if (docStatusValue === 'Pending') matchesDocStatus = ds === 'Pending';
                else if (docStatusValue === 'Out')     matchesDocStatus = ds === 'Out';
            }
    ...
    displayEntries() {
        ...
        entriesTableBody.innerHTML = pageEntries.map(entry => {
            ...
            // ─ Document status pill (In / Pending / Out)
            const rawDs = entry.docStatus || 'In';
            const ds = rawDs.toLowerCase() === 'incoming' ? 'In' : (rawDs.toLowerCase() === 'pending' || rawDs.toLowerCase() === 'in-service' ? 'Pending' : rawDs);
            
            let docStatusPill = '';
            if (ds === 'In') {
                docStatusPill = `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-[9px] font-black uppercase tracking-wider border border-blue-200 dark:border-blue-800"><div class="w-1.5 h-1.5 rounded-full bg-blue-500"></div>Incoming</span>`;
            } else if (ds === 'Pending') {
                ...
                docStatusPill = `<span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-400 text-[9px] font-black uppercase tracking-wider border border-emerald-300 dark:border-emerald-700"><div class="w-1.5 h-1.5 rounded-full bg-emerald-500"></div>Out / Done</span>`;
            }
    ...
    async updateStats() {
        try {
            ...
            this.entries.forEach(entry => {
                ...
                const ds = rawDs.toLowerCase() === 'incoming' ? 'In' : (rawDs.toLowerCase() === 'pending' || rawDs.toLowerCase() === 'in-service' ? 'Pending' : rawDs);
                if (ds === 'In') inCount++;
                else if (ds === 'Pending') pendingCount++;
                else outCount++;
    ...
    async loadPendingRequests() {
        ...
        const pending = this.entries.filter(e => {
            const rawDs = e.docStatus || 'In';
            const ds = rawDs.toLowerCase() === 'incoming' ? 'In' : (rawDs.toLowerCase() === 'pending' || rawDs.toLowerCase() === 'in-service' ? 'Pending' : rawDs);
            return ds === 'Pending' || ds === 'In';
        });
        ...
        tbody.innerHTML = pending.map(e => {
            ...
            const ds = rawDs.toLowerCase() === 'incoming' ? 'In' : (rawDs.toLowerCase() === 'pending' || rawDs.toLowerCase() === 'in-service' ? 'Pending' : rawDs);
            const pill = ds === 'In'
                ? `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-[9px] font-black uppercase border border-blue-200"><div class="w-1 h-1 rounded-full bg-blue-500"></div>Incoming</span>`
    ...
    async singleApprove(entryId, newStatus) {
        try {
            const res = await fetch(`/api/logs/${entryId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ docStatus: newStatus, officeId: this.officeId })
            });
            if (res.ok) {
                this.showToast(`Document status updated to ${newStatus}.`);
    ...
    async bulkUpdateStatus(newStatus) {
        ...
        this.showToast(`Updating ${checkboxes.length} request(s)…`);

        const promises = checkboxes.map(cb =>
            fetch(`/api/logs/${cb.dataset.entryId}`, {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ docStatus: newStatus, officeId: this.officeId })
            })
        ...
        try {
            ...
            this.showToast(`${checkboxes.length} request(s) marked as ${newStatus}.`);
