...
// Helper to send visit-done notification email
async function sendVisitDoneNotification(studentEmail, studentName, activity) {
    if (!studentEmail || !process.env.SMTP_USER) {
        console.warn('⚠️ Skipping visit-done email: Student email or SMTP credentials missing.');
        return;
    }
    ...
    const mailOptions = {
        from: process.env.EMAIL_FROM || `"Logbook System" <${process.env.SMTP_USER}>`,
        to: studentEmail,
        subject: '✅ Your Visit Has Been Completed',
        html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden;">
                <div style="background-color: #16a34a; color: white; padding: 24px; text-align: center;">
                    <h1 style="margin: 0; font-size: 20px;">Visit Completed</h1>
                </div>
                <div style="padding: 24px; color: #1e293b; line-height: 1.6;">
                    <p>Hi <strong>${studentName}</strong>,</p>
                    <p>Your visit to the <strong>${process.env.OFFICE_NAME || 'Engineering Office'}</strong> for <strong>${activity}</strong> has been marked as <strong>completed</strong>.</p>
                    <p>Thank you for your visit. If you have any follow-up concerns, please don't hesitate to return to the office.</p>
                    <hr style="border: 0; border-top: 1px solid #e2e8f0; margin: 24px 0;">
                    <p style="font-size: 12px; color: #64748b;">This is an automated notification from the Logbook System. Please do not reply to this email.</p>
                </div>
            </div>
...
app.patch('/api/logs/:id/status', async (req, res) => {
    ...
    try {
        ...
        // Send email alert when visit is marked done/completed
        if (status === 'done' || status === 'completed') {
            ...
            if (log) {
                ...
                if (targetEmail) {
                    sendVisitDoneNotification(targetEmail, log.studentName, log.activity);
                }
            }
        }
    } catch (error) {
