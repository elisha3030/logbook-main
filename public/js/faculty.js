                ...
                <td class="px-6 py-4 text-center">
                    ...
                    ${(() => {
                        if ((log.activity || '').toLowerCase().startsWith('document request') && !isCompleted) {
                            ...
                            const ds = rawDs.toLowerCase() === 'incoming' ? 'In' : (rawDs.toLowerCase() === 'pending' || rawDs.toLowerCase() === 'in-service' ? 'Pending' : rawDs);
                            ...
                            return `
                                <div class="flex items-center justify-center gap-1 mt-2" onclick="event.stopPropagation()">
                                    ...
                                    <button onclick="window.updateDocStatus('${log.id}', 'Pending')" title="Mark as Pending" 
                                        class="w-6 h-6 rounded shadow-sm bg-amber-100 text-amber-700 flex items-center justify-center text-[8px] font-black hover:bg-amber-200 transition-all ${ds === 'Pending' ? activeClass : 'opacity-40'}">PND</button>
...
window.viewTransactionDetails = function(logId) {
    ...
    const isDocRequest = (log.activity || '').toLowerCase().startsWith('document request');
    const isCompleted = log.status === 'completed';
