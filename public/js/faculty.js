...
async function fetchQueue() {
    try {
        ...
        const pending = allLogs.filter(l => {
            if (l.studentNumber === 'EMPLOYEE_LOG') return false;
            ...
            // Document Requests remain in the queue even after student timeOut.
            if (isDocRequest) return true;

            // Other transactions are only considered pending while the visit is active.
            return l.timeOut === null;
        });
        ...
        const result = { pending, completed, staffPresence, all: [...pending, ...completed] };
        currentQueue = result.all; // Cache for detail view
        return result;
    } catch (e) {
...
async function renderQueue() {
    ...
    tbody.innerHTML = sorted.map(log => {
        ...
        // For non-doc-request visits, a timeOut means the visit is done even if status wasn't updated.
        const isCompleted = rs === 'completed' || (!isDocRequest && !!log.timeOut);
        const isInService = !isCompleted && rs === 'in-service';
        const isPending = !isCompleted && !isInService;
        ...
        return `
            <tr onclick="window.viewTransactionDetails('${escape(log.id)}')" 
                ...
                <td class="px-6 py-4 text-center">
                    ${statusBadge}
                    ${(() => {
                        if ((log.activity || '').toLowerCase().startsWith('document request') && !isCompleted) {
                            const rawDs = log.docStatus || 'In';
                            const ds = rawDs.toLowerCase() === 'incoming' ? 'In' : rawDs;
                            const activeClass = 'ring-2 ring-blue-500 scale-105 z-10 opacity-100';
                            return `
                                <div class="flex items-center justify-center gap-1 mt-2" onclick="event.stopPropagation()">
                                    <button onclick="window.updateDocStatus('${log.id}', 'In')" title="Mark as In" 
                                        class="w-6 h-6 rounded shadow-sm bg-blue-100 text-blue-700 flex items-center justify-center text-[8px] font-black hover:bg-blue-200 transition-all ${ds === 'In' ? activeClass : 'opacity-40'}">IN</button>
                                    <button onclick="window.updateDocStatus('${log.id}', 'Out')" title="Mark as Out" 
                                        class="w-6 h-6 rounded shadow-sm bg-emerald-100 text-emerald-700 flex items-center justify-center text-[8px] font-black hover:bg-emerald-200 transition-all ${ds === 'Out' ? activeClass : 'opacity-40'}">OUT</button>
                                </div>
                            `;
                        }
                        return '';
...
// ----------------------------------------------------------------
window.updateDocStatus = async function (logId, newStatus) {
    try {
        const res = await fetch(`/api/logs/${logId}/doc-status`, {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ docStatus: newStatus, officeId })
        });
...
async function openReportModal() {
    ...
    const avgSvcEl = document.getElementById('reportAvgSvc');
    const peakEl = document.getElementById('reportPeak');
    ...
    if (avgSvcEl) {
        const completedWithSvc = completed.filter(l => l.serviceStartTime && l.timeOut);
        if (completedWithSvc.length > 0) {
            const sumSvcMs = completedWithSvc.reduce((sum, l) => sum + (new Date(l.timeOut) - new Date(l.serviceStartTime)), 0);
            const avgMin = Math.round((sumSvcMs / completedWithSvc.length) / 60000);
            avgSvcEl.textContent = `${avgMin}m`;
        } else {
            avgSvcEl.textContent = '—';
        }
    }

    if (peakEl) {
        const hrCounts = {};
        all.forEach(l => { if (l.timeIn) { const hr = new Date(l.timeIn).getHours(); hrCounts[hr] = (hrCounts[hr] || 0) + 1; } });
        const busiestHr = Object.entries(hrCounts).sort((a, b) => b[1] - a[1])[0]?.[0];
        peakEl.textContent = busiestHr !== undefined ? new Date(0, 0, 0, parseInt(busiestHr)).toLocaleTimeString([], { hour: 'numeric', hour12: true }) : '—';
    }

    // 2. Activity List
    if (activityList) {
        const counts = {};
        all.forEach(l => { counts[l.activity] = (counts[l.activity] || 0) + 1; });
        const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 3);
        
        if (sorted.length === 0) {
            activityList.innerHTML = '<p class="text-[11px] text-slate-400 italic">No activities logged today.</p>';
        } else {
            const max = sorted[0][1];
            activityList.innerHTML = sorted.map(([act, count]) => {
                const pct = Math.round((count / max) * 100);
                return `
                    <div class="space-y-1">
                        <div class="flex justify-between text-[10px] font-black uppercase tracking-wider">
                            <span class="text-slate-600 dark:text-slate-300 truncate max-w-[80%]">${escape(act)}</span>
    ...
    // 3. Log Body (Table)
    if (logBody) {
        // Show last 10 entries for conciseness
        const displayLogs = all.slice(0, 10);
        if (displayLogs.length === 0) {
            logBody.innerHTML = '<tr><td colspan="3" class="px-4 py-8 text-center text-slate-400 italic">No logs found</td></tr>';
        } else {
            logBody.innerHTML = displayLogs.map(l => `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                    <td class="px-4 py-3">
                        <p class="font-bold text-slate-700 dark:text-slate-200 leading-none">${escape(l.studentName || '—')}</p>
                        <p class="text-[9px] text-slate-400 font-mono mt-0.5">${escape(l.studentId || l.studentNumber || '')}</p>
                    </td>
                    <td class="px-4 py-3 text-slate-500 dark:text-slate-400">${escape(l.activity || '—')}</td>
                    <td class="px-4 py-3 text-right">
                        <p class="font-bold text-slate-700 dark:text-slate-200">${formatTime(l.timeIn)}</p>
                        <p class="text-[9px] text-slate-400">${l.timeOut ? formatTime(l.timeOut) : 'Active'}</p>
                    </td>
                </tr>
            `).join('');
        }
...
/**
 * viewTransactionDetails (logId)
 * Opens a modal with full details of a specific visit.
 */
window.viewTransactionDetails = function(logId) {
    const log = currentQueue.find(l => String(l.id) === String(logId));
    if (!log) return;
