...
async function sendStudentNotification(targetEmail, studentName, activity, type = 'ready') {
    ...
    if (type === 'received') {
        ...
        bodyText = `Your transaction for <strong>${activity}</strong> at the <strong>${process.env.OFFICE_NAME || 'Engineering Office'}</strong> has been marked as complete.`;
    ...
    const mailOptions = {
        ...
        html: `
            <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                <div style="background-color: #2563eb; color: white; padding: 32px 24px; text-align: center;">
                    <h1 style="margin: 0; font-size: 24px; font-weight: 800; letter-spacing: -0.025em;">${headerText}</h1>
                </div>
                <div style="padding: 32px 24px; color: #1e293b; line-height: 1.6;">
                    ...
                    <div style="text-align: center;">
                        ...
                        <p style="font-size: 11px; color: #cbd5e1; margin-top: 4px;">Please do not reply to this email.</p>
                    </div>
                </div>
            </div>
        `
...
app.patch('/api/logs/bulk-complete', async (req, res) => {
    try {
        ...
        // 4. Send email notifications for each completed log
        (async () => {
            try {
                ...
                console.error('❌ Error sending bulk completion emails:', err);
            }
        })();
    } catch (error) {
...
app.patch('/api/logs/:id', async (req, res) => {
    try {
        ...
        if (docStatus) {
            // Admin bulk/single document location update (In / Out)
            const normalized = String(docStatus || '').trim();
            const safeDocStatus = normalized === 'Pending' ? 'In' : normalized;
            if (safeDocStatus !== 'In' && safeDocStatus !== 'Out') {
                return res.status(400).json({ error: "docStatus must be 'In' or 'Out'" });
            }
            await localDb.run(
...
app.patch('/api/logs/:id/claim', async (req, res) => {
    try {
        const { id } = req.params;
        const { studentNumber, officeId } = req.body || {};
        if (!studentNumber) return res.status(400).json({ error: 'studentNumber required' });

        const log = await localDb.get(
            'SELECT id, studentNumber, officeId, activity, status, docStatus FROM logs WHERE id = ?',
            id
        );
        if (!log) return res.status(404).json({ error: 'Log not found' });
        if (String(log.studentNumber) !== String(studentNumber)) {
            return res.status(403).json({ error: 'Not allowed to claim this request' });
        }
        if (officeId && String(log.officeId || '') !== String(officeId)) {
            return res.status(403).json({ error: 'Office mismatch' });
        }
        if (!isDocumentRequestActivity(log.activity)) {
            return res.status(400).json({ error: 'Only document requests can be claimed' });
        }

        const currentStatus = String(log.status || '').toLowerCase();
        if (currentStatus !== 'completed') {
            return res.status(400).json({ error: 'Document is not marked as completed yet' });
        }

        const currentLoc = String(log.docStatus || '').toLowerCase();
        if (currentLoc === 'out') {
            return res.json({ success: true, alreadyClaimed: true });
        }

        await localDb.run(
            "UPDATE logs SET docStatus = 'Out', synced = 0 WHERE id = ?",
            [id]
        );
        ...
        console.error('Error claiming document request:', error);
...
app.patch('/api/logs/:id/service-start', async (req, res) => {
    try {
        ...
        if (log && (isDocumentRequestActivity(log.activity) || isDocumentSubmissionActivity(log.activity))) {
            ...
            if (targetEmail) {
                sendStudentNotification(targetEmail, log.studentName, log.activity, 'processing');
            }
        }
    } catch (error) {
...
// Update document location (In, Out)
app.patch('/api/logs/:id/doc-status', async (req, res) => {
    try {
        ...
        // If location is 'Out', notify the student via email
        if (safeDocStatus === 'Out') {
            const log = await localDb.get('SELECT * FROM logs WHERE id = ?', id);
            if (log) {
...
app.delete('/api/logs/clear-active', requireAdmin, async (req, res) => {
    try {
        const timeOut = new Date().toISOString();
        const docReq = await localDb.run(
            "UPDATE logs SET timeOut = ?, docStatus = COALESCE(docStatus, 'In'), synced = 0 WHERE timeOut IS NULL AND LOWER(COALESCE(activity,'')) LIKE 'document request%'",
            [timeOut]
        );
        const docPick = await localDb.run(
            "UPDATE logs SET timeOut = ?, status = 'completed', docStatus = 'Out', synced = 0 WHERE timeOut IS NULL AND (LOWER(COALESCE(activity,'')) LIKE 'document pick-up%' OR LOWER(COALESCE(activity,'')) LIKE 'document pickup%')",
            [timeOut]
        );
        const other = await localDb.run(
            "UPDATE logs SET timeOut = ?, docStatus = 'Out', synced = 0 WHERE timeOut IS NULL AND LOWER(COALESCE(activity,'')) NOT LIKE 'document request%' AND LOWER(COALESCE(activity,'')) NOT LIKE 'document pick-up%' AND LOWER(COALESCE(activity,'')) NOT LIKE 'document pickup%'",
            [timeOut]
        );

        const cleared = (docReq?.changes || 0) + (docPick?.changes || 0) + (other?.changes || 0);
        console.log(`🧹 Cleared ${cleared} stuck active session(s). (doc-requests kept pending: ${docReq?.changes || 0}, doc-pickups completed: ${docPick?.changes || 0})`);
    ...
    autoCheckoutTimer = setInterval(async () => {
        try {
            ...
            if (now.getHours() === hh && now.getMinutes() === mm) {
                ...
                const closed = (docReq?.changes || 0) + (docPick?.changes || 0) + (other?.changes || 0);
                if (closed > 0) {
                    console.log(`⏰ Auto-checkout: Closed ${closed} active session(s) at ${checkoutTime} (doc-requests kept pending: ${docReq?.changes || 0}, doc-pickups completed: ${docPick?.changes || 0})`);
                    syncToCloud();
                }
            }
        } catch (e) { /* skip */ }
